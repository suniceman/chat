...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}'
      ,'<li>'
        ,'<h5 id="{{item.id}}" layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
  ...
  var banRightMenu = function(){
    ...
    layimMain.find('.layim-list-friend').on('contextmenu', 'h5', function (e) {
        ...
        html += '<li layim-event="group_rename">' + '重命名</li></ul>';
        html += '<li layim-event="group_deleted">' + '删除分组</li>';
        html += '<li layim-event="group_created">' + '新建分组</li></ul>';
        if (othis.hasClass('layim-null')) return;
        //注意一下这个方法
        layer.tips(html, this, {
            tips: 1
...
//        showtips(html, this, id, 0, 0);
  ...
  //好友右键菜单
    layimMain.find('.layim-list-friend').on('contextmenu', '.layui-layim-list li', function (e) {
        ...
        var html = '<ul id="contextmenu_' + othis[0].id + '" data-id="' + othis[0].id + '" data-index="' + othis.data('index') + '">';
        html += '<li layim-event="menu_msg">' + '发送消息</li>';
