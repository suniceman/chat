...
public class EHCacheUtil {
    ...
    public <K, V> CacheManager getCacheManager(Class<K> kClass,
            Class<V> vClass, String configureName) {
    ...
    public <K, V> Cache<K, V> getCache(Class<K> kClass, Class<V> vClass,
            String cacheName, String configureName, CacheManager manager) {
        
        Cache<K, V> cache = manager.getCache(configureName, kClass, vClass);
        /*
         * Cache<K, V> cache = manager.createCache(cacheName,
         * CacheConfigurationBuilder.newCacheConfigurationBuilder(kClass,
         * vClass, ResourcePoolsBuilder.heap(10)).build());
         */
        return cache;
    }
